...
export default function Scanner({ setScanResult, onNavigate }) {
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      // Auto-analyze protocol stacks
      if (r.id) {
        analyzeProtocols(r.id).then(setProtocolStacks).catch(() => {});
      }
        ...
        <>
            ...
            <motion.div variants={fadeUp}>
              <ProCard title={
                <span>
                  ...
                  协议栈检测 (TLS = KEM + Sig + AEAD)
                  <Badge count={protocolStacks.total_stacks} style={{ marginLeft:12, backgroundColor:'#ffa502' }} />
                </span>
              } className="qs-card" headerBorderless>
                <div style={{ display:'flex', gap:8, marginBottom:16, flexWrap:'wrap' }}>
                  <Tag color="blue">TLS 协议栈: {protocolStacks.tls_stacks}</Tag>
                  <Tag color="purple">自定义协议: {protocolStacks.custom_stacks}</Tag>
                </div>
                {protocolStacks.protocol_stacks.map((stack, i) => (
                  <div key={i} style={{ padding:16, marginBottom:12, background:'#0a0a20', borderRadius:10, border:`1px solid ${stack.risk === 'CRITICAL' ? 'rgba(255,71,87,0.3)' : 'rgba(255,165,2,0.3)'}`, borderLeft:`4px solid ${stack.risk === 'CRITICAL' ? '#ff4757' : '#ffa502'}` }}>
                    <div style={{ display:'flex', justifyContent:'space-between', marginBottom:8, flexWrap:'wrap', gap:8 }}>
                      ...
                      <Text style={{ color:'#555577', fontSize:11 }}>{stack.finding_count} 发现</Text>
                    </div>
                    <Row gutter={12}>
                      <Col span={8}>
                        <div style={{ padding:8, background:'rgba(108,92,231,0.06)', borderRadius:6 }}>
                          <Text style={{ color:'#7a7a9e', fontSize:10, display:'block' }}>KEM (密钥封装)</Text>
                          {stack.components.kem.length > 0
                            ? stack.components.kem.map((a,j) => <Tag key={j} color="red" style={{ fontSize:10, margin:'2px 2px 0 0' }}>{a}</Tag>)
                            : <Text style={{ color:'#4a4a6e', fontSize:11 }}>未检测到</Text>}
                          <div style={{ marginTop:4 }}>
                            <ArrowRightOutlined style={{ color:'#2ed573', fontSize:9, marginRight:4 }} />
                            <Text style={{ color:'#2ed573', fontSize:10 }}>{stack.migration_target.kem}</Text>
                          </div>
                        </div>
                      </Col>
                      <Col span={8}>
                        <div style={{ padding:8, background:'rgba(108,92,231,0.06)', borderRadius:6 }}>
                          <Text style={{ color:'#7a7a9e', fontSize:10, display:'block' }}>签名 (Signature)</Text>
                          {stack.components.signature.length > 0
                            ? stack.components.signature.map((a,j) => <Tag key={j} color="red" style={{ fontSize:10, margin:'2px 2px 0 0' }}>{a}</Tag>)
                            : <Text style={{ color:'#4a4a6e', fontSize:11 }}>未检测到</Text>}
                          <div style={{ marginTop:4 }}>
                            <ArrowRightOutlined style={{ color:'#2ed573', fontSize:9, marginRight:4 }} />
                            <Text style={{ color:'#2ed573', fontSize:10 }}>{stack.migration_target.signature}</Text>
                          </div>
                        </div>
                      </Col>
                      <Col span={8}>
                        <div style={{ padding:8, background:'rgba(108,92,231,0.06)', borderRadius:6 }}>
                          <Text style={{ color:'#7a7a9e', fontSize:10, display:'block' }}>AEAD (对称加密)</Text>
                          {stack.components.aead.length > 0
                            ? stack.components.aead.map((a,j) => <Tag key={j} color={a.includes('AES-256') ? 'green' : 'orange'} style={{ fontSize:10, margin:'2px 2px 0 0' }}>{a}</Tag>)
                            : <Text style={{ color:'#4a4a6e', fontSize:11 }}>未检测到</Text>}
                          <div style={{ marginTop:4 }}>
                            <ArrowRightOutlined style={{ color:'#2ed573', fontSize:9, marginRight:4 }} />
                            <Text style={{ color:'#2ed573', fontSize:10 }}>{stack.migration_target.aead}</Text>
                          </div>
                        </div>
                      </Col>
                    </Row>
          ...
          <motion.div variants={fadeUp}>
            <ProCard title={
              ...
              <Row gutter={16}>
                <Col xs={24} md={12}>
                  <div style={{ marginBottom:8 }}><Text strong style={{ color:'#ccc', fontSize:13 }}>按算法聚合</Text></div>
                  {Object.entries(result.summary.by_algorithm || {}).sort((a,b) => b[1] - a[1]).map(([algo, count], i) => (
                    <div key={i} style={{ display:'flex', alignItems:'center', gap:8, padding:'6px 0', borderBottom:'1px solid rgba(26,26,66,0.3)' }}>
                      <Tag color="red" style={{ fontSize:10, minWidth:90, textAlign:'center' }}>{algo}</Tag>
                      <Progress percent={Math.round((count / result.summary.total_findings) * 100)} size="small" strokeColor="#6C5CE7" trailColor="rgba(26,26,66,0.6)" style={{ flex:1 }} />
                      <Text style={{ color:'#7a7a9e', fontSize:11, minWidth:30, textAlign:'right' }}>{count}</Text>
                    </div>
                  ))}
                </Col>
                <Col xs={24} md={12}>
                  <div style={{ marginBottom:8 }}><Text strong style={{ color:'#ccc', fontSize:13 }}>按用途类型分布</Text></div>
                  {Object.entries(result.summary.by_usage_type || {}).sort((a,b) => b[1] - a[1]).map(([usage, count], i) => (
                    <div key={i} style={{ display:'flex', alignItems:'center', gap:8, padding:'6px 0', borderBottom:'1px solid rgba(26,26,66,0.3)' }}>
                      ...
                      <Progress percent={Math.round((count / result.summary.total_findings) * 100)} size="small" strokeColor="#ffa502" trailColor="rgba(26,26,66,0.6)" style={{ flex:1 }} />
                      <Text style={{ color:'#7a7a9e', fontSize:11, minWidth:30, textAlign:'right' }}>{count}</Text>
                    </div>
                  ))}
                  {result.summary.by_library && Object.keys(result.summary.by_library).length > 0 && (
                    <>
                      <div style={{ marginTop:16, marginBottom:8 }}><Text strong style={{ color:'#ccc', fontSize:13 }}>依赖库引入</Text></div>
                      {Object.entries(result.summary.by_library).sort((a,b) => b[1] - a[1]).map(([lib, count], i) => (
                        <div key={i} style={{ display:'flex', alignItems:'center', gap:8, padding:'4px 0' }}>
                          <Tag color="purple" style={{ fontSize:10 }}>{lib}</Tag>
                          <Text style={{ color:'#7a7a9e', fontSize:11 }}>{count} 次引用</Text>
                        </div>
                      ))}
                    </>
                  )}
